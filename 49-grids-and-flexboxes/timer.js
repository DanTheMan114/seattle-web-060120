...
let instrs = ["beans", "river", "viri", "wedge", "sig", "rose", "elio", "max", "reilley", "tabitha", "dot", "stella", "chai", "imber", "jackson"];
...
function addInstrs() {
    for (let i = 0; i < instrs.length; i++) {
        ...
        start.innerText = `Start ${name}`
        start.id = `${name} start`
        start.addEventListener("click", () => startTimer(name))

        let end = document.createElement('button')
        end.innerText = "End"
        end.id = `${name} end`
        end.className = 'hidey'
        end.addEventListener("click", () => endTimer(name))
