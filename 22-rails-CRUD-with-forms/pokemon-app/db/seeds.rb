Pokemon.delete_all
# Team.delete_all
# Trainer.delete_all

pikachu = Pokemon.create(name: "Pikachu", level: 52, skill: "Thunder bolt", ability: "Lightening rod", image_url: "https://assets.pokemon.com/assets/cms2/img/pokedex/full/025.png" )
bulbasaur = Pokemon.create(name: "Bulbasaur", level: 55, skill: "Solar Bean", ability: "Leaf Guard", image_url: "https://i.pinimg.com/736x/6f/aa/74/6faa74e32ce5f00f2dbc9c0b095cda52.jpg")
charmander = Pokemon.create(name: "Charmander", level: 42, skill: "Flame Thrower", ability: "Flash-fire", image_url: "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTGpgd3E2JDVYcsyqn_qMz67IrnOsB0TdrJgA&usqp=CAU")
squirtle = Pokemon.create(name: "Squirtle", level: 44, skill: "Waterfall", ability: "Liquid Voice", image_url: "https://cdn.bulbagarden.net/upload/3/39/007Squirtle.png")
dargonite = Pokemon.create(name: "Dragonite", level: 88, skill: "Dragon Breath", ability: "Victory Star", image_url: "https://img.pokemondb.net/artwork/large/dragonite.jpg")
# gengar = Pokemon.create(name: "Gengar", level: 77, skill: "Night Shade", ability: "Soul-Heart")
# magmar = Pokemon.create(name: "Magmar", level: 66, skill: "Flame Wheel", ability: "Vital Spirit")
# lucario = Pokemon.create(name: "Lucario", level: 78, skill: "Double Kick", ability: "Inner-Focus")
# weavile = Pokemon.create(name: "Weavile", level: 66, skill: "Night Slash", ability: "Pressure")
# milotic = Pokemon.create(name: "Milotic", level: 58, skill: "Aqua Tail", ability: "Marvel Scale")
# absol = Pokemon.create(name: "Absol", level: 88, skill: "Dark Pulse", ability: "Super Luck")
# greninja = Pokemon.create(name: "Greninja", level: 85, skill: "Darkest Lariat", ability: "Torrent")
# mudsdale = Pokemon.create(name: "Mudsdale", level: 79, skill: "High Horsepower", ability: "Own Tempo")
# tyrantrum = Pokemon.create(name: "Tyrantrum", level: 90, skill: "Clanging Scales", ability: "Strong Jaw")
# empoleon = Pokemon.create(name: "Empoleon", level: 95, skill: "Razor Shell", ability: "Torrent")
# arcanine = Pokemon.create(name: "Arcanine", level: 84, skill: "Inferno", ability: "Flash Fire")
# snorlax = Pokemon.create(name: "Snorlax", level: 66, skill: "Body Slam", ability: "Thick Fat")
# garchomp = Pokemon.create(name: "Garchomp", level: 95, skill: "Dragon Tail", ability: "Sand Veil")
# corviknight = Pokemon.create(name: "Corviknight", level: 44, skill: "Waterfall", ability: "Unnerve")

# ash = Trainer.create(name: "Ash", age: 15, hometown: "Pallet Town")
# misty = Trainer.create(name: "Misty", age: 16, hometown: "aqua town")
# brock = Trainer.create(name: "Brock", age: 17, hometown: "Rock Town")

  # binding.pry


  puts "all seeds are done. woot!"
